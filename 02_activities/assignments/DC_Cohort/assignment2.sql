...
--QUERY 2
--with the counter changing on each new market date for each customer, use ROW_NUMBER ()
SELECT 
 *,
 ROW_NUMBER () OVER (
...
--select only the unique market dates per customer(without purchase details) and number those visits, use DENSE_RANK()
SELECT
 customer_id,
 market_date,
 DENSE_RANK () OVER (
...
--QUERY 7
--check rank: 2022-12-20, 979, rank 1; 2023-04-07, 272.5, rank 142
--ranked_sales AS (
-- SELECT
--  market_date,
--  total_sales,
...
--DROP TABLE product_units; --for practice
--END QUERY
...
WHERE product_name = 'Maple cookies';
--END QUERY
...
--Find the latest quantity per product
SELECT 
 vi.product_id,
...
ON vi.product_id = t.product_id
AND vi.market_date = t.last_date;
